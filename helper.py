...
def create_one_vs_one(df1, df2, class_size):
    return pd.concat([df1[:class_size], df2[:class_size]]).reset_index(drop=True).copy()
...
def loadFromDisk():
    #return np.genfromtxt('features.csv', delimiter=' ', dtype=np.float64)
